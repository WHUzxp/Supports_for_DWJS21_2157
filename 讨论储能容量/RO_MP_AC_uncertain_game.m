function result_MP=RO_MP_DC_uncertain_game(data_SP,percent_ESS)
%% 基础数据
N=50;%电动汽车数量
...
ratio_zhuanhuan=[];
for i=1:Iterations
    ratio_zhuanhuan=[ratio_zhuanhuan,ratio(:,i)*ones(1,10)];
end
%电动汽车状态矩阵
...
Pch=sdpvar(24,10*Iterations);%储能系统充电
Pdis=sdpvar(24,10*Iterations);%储能系统放电
S_ESS=sdpvar(24,10*Iterations);%储能系统电量状态
Pb_DA=sdpvar(24,1);%日前购电合同
price_DA=a_data.*Pb_DA+b_data;%日前DLMP
Pb_RT=sdpvar(24,10*Iterations);%实时购电量
Ps_RT=sdpvar(24,10*Iterations);%实时售电量
C_price=[mean(price_EV)<=mean(price_DA),0.8*price_DA<=price_EV<=1.2*price_DA];%零售电价约束条件
C_ESS=[0<=Pch<=250*percent_ESS,0<=Pdis<=250*percent_ESS,200*percent_ESS<=S_ESS<=950*percent_ESS,
    S_ESS(1,:)==500*percent_ESS+0.95*Pch(1,:)-Pdis(1,:)/0.95,
    S_ESS(2:24,:)==S_ESS(1:23,:)+0.95*Pch(2:24,:)-Pdis(2:24,:)/0.95,
    S_ESS(24,:)==500*percent_ESS];%储能系统约束条件
C_CS=[0<=Pb_DA<=500,0<=Pb_RT<=500,0<=Ps_RT<=500,Pb_DA*ones(1,10*Iterations)+Pb_RT+Pdis+N*pdis*ratio_zhuanhuan==Ps_RT+Pch+N*pch*ratio_zhuanhuan];%零售商约束条件
obj_CCG=sdpvar;%引入一个辅助变量用于CCG迭代
C_CCG=[];
for i=1:Iterations
    C_CCG=[C_CCG,obj_CCG<=sum(PDF.*sum(-(price_RT+0.001).*Pb_RT(:,10*i-9:10*i)+(price_RT-0.001).*Ps_RT(:,10*i-9:10*i)))+N*obj_EV*ratio(:,i)];
end
%% 求解
